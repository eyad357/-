...
(async () => {
  ...
  await check('policy: pptx preview fidelity is "full-if-available" (real layout rendering when LibreOffice present, honest fallback otherwise)', async () => {
    const p = FileSupportPolicy.getPolicy('deck.pptx');
    if (p.preview.fidelity !== 'full-if-available') throw new Error('expected full-if-available, got ' + p.preview.fidelity);
  });
