...
(function (root, factory) {
  ...
  const EXTENSIONS = {
    ...
    pptx: {
      ...
      upload: { allowed: true }, maxSizeBytes: 100 * MB,
      preview: { supported: true, engine: 'office-conversion', fidelity: 'full-if-available' },
      contentExtraction: { supported: true }, thumbnail: { supported: false, engine: null },
      search: { supported: true }, ocr: { supported: false },
      displayNameAr: 'عرض PowerPoint', fallback: 'preview', signature: SIG.ZIP,
      notes: 'Primary preview is real slide-layout rendering: converted to PDF via a locally-installed headless LibreOffice (server/services/officeConversionService.js), then displayed through the same PDFEngine used for .pdf files, in a dedicated single-slide-at-a-time presentation-mode viewer. Fidelity is genuinely full (background, fonts, shapes, tables, exact positioning) WHEN LibreOffice is present on the machine — this is a real rendering engine doing real layout, not an approximation. If LibreOffice is not installed, or conversion fails for a specific file, falls back to the older JSZip/DOMParser text+image extraction (partial fidelity, clearly labeled as such in the UI) rather than failing outright. See PPTX-PRESENTATION-MODE-REPORT.md.',
    },
    ppt: {
